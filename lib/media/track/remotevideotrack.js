...
const hasDocumentPiP = require('../../utils/hasdocumentpip');
const playAllAttachedTracks = require('../../util/playattachedtracks');
...
function maybeUpdateEnabledHint(remoteVideoTrack) {
  ...
  const visibleWithinDocPiP = document.visibilityState === 'hidden' && hasDocumentPiP() && visibleElements.length > 0;
  const visibleDocWithVisibleElement = document.visibilityState === 'visible' && visibleElements.length > 0;

  // even when document is invisible we may have track playing in pip window.
  const enabled = visibleDocWithVisibleElement || visibleWithinDocPiP || pipWindows.length > 0;
